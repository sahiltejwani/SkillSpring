...
const SubSectionModal = ({
    ...
    useEffect(() => {
        if (view || edit) {
            ...
            setValue("lectureDur", modalData.timeDuration);
    ...
    const isFormUpdated = () => {
        ...
        if (currentValues.lecture !== modalData.title ||
            ...
            currentValues.lectureDur !== modalData.timeDuration
        ) {
            return true;
        }
        return false;
    ...
    const handelEditSubsection = async (data) => {
        ...
        if(currentValues.lectureDur !== modalData.timeDuration) {
            formData.append("timeDuration", data.lectureDur);
        }
        ...
        <div className='fixed inset-0 z-[1000] !mt-0 grid h-screen w-screen place-items-center overflow-auto bg-white bg-opacity-10 backdrop-blur-sm'>
            <div className='my-10 w-11/12 max-w-[700px] rounded-lg border border-richblack-400 bg-richblack-800'>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 px-8 py-10">
                    ...
                    <div className='flex flex-col space-y-2'>
                        <label className='text-sm text-richblack-5'>Lecture Duration (in seconds)</label>
                        <input
                            disabled={view}
                            id='timeDuration'
                            placeholder='Enter Duration in seconds'
                            {...register("lectureDur", { required: true })}
                            className='form-style w-full'
                            type='number'
                            min='0'
                        />
